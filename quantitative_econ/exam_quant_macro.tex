...
\section*{Q3}
The value of $\beta$ that makes the capital output ratio equal to $3.2$ is \textbf{0.967}, slighlty higher than 0.96 from the original specification. \\
...
Figure \ref{fig:ev_tau_1} shows the labor income tax rate $\tau_w(b)$ that balances the government budget for each value of $b$. Since higher unemployment benefits require higher government expenditures, the balanced-budget condition implies an increasing and linear relationship between $b$ and $\tau_w(b)$. \\

Figure \ref{fig:ev_b_1} reports the consumption-equivalent variation $\Delta CEV(b)$ relative to the baseline economy with $b=0.35$. By construction, $\Delta CEV(0.35)=0$. For $b>0.35$, welfare decreases monotonically, whereas for $b<0.35$ the CEV is positive. This pattern reflects the fact that unemployment is relatively rare an households can self-insure through savings. As a result, the increase in the distortionary labor tax needed to finance more generous b reduces welfare during the (more frequent) employment spells by more than the additional unit of b raise welfare during unemployment spells. Consequently, under our exercise, the balanced-budget optimal employment benefit lies below the empirical value of $b=0.35$. \\
...
\xi(x) = 1 - \exp(-\kappa x),
\]
and we define the value differential, as given above:
\[
...
\]
Since $V(a',s',0)$ does not depend on $x$, the problem reduces to simplh:
\[
\max_{x \ge 0} \; 
...
\]
Letting $y \equiv \kappa x$, we obtain the final expression:
\[
...
\section*{Q3}
I first calibrate $\kappa$ keeping $\beta$ fixed and using the calibrated $\beta$ (i.e $0.967$) as reference. I find that the values of $\kappa$ that make job finding rate equal to $0.55$ are between $1.20-1.22$. The capital output ratio is very close to the desired range with these two values and with the guessed $\beta$, so I avoid guessing this parameter for the fixed optimal $\kappa$. Therefore, for my last exercise, I use the following calibrated parameter: 
